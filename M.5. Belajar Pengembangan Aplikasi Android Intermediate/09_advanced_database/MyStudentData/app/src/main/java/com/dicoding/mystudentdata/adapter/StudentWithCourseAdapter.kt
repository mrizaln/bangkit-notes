...
class StudentWithCourseAdapter :
    ...
    class WordViewHolder(private val binding: ItemStudentBinding) :
        ...
        fun bind(data: StudentWithCourse) {
            ...
            binding.tvItemUniversity.apply {
                text = data.studentAndUniversity.university?.name
                visibility = View.VISIBLE
            }
            val arrayCourse = data.course.map { it.name }
            binding.tvItemCourse.apply {
                text = arrayCourse.joinToString(separator = ", ")
    ...
    class WordsComparator : DiffUtil.ItemCallback<StudentWithCourse>() {
        override fun areItemsTheSame(
            oldItem: StudentWithCourse, newItem: StudentWithCourse,
        ): Boolean {
